...
create_app["create_app()<br>src/app/init.py"]
...
sequenceDiagram
  ...
  api_blueprint->>api_blueprint: (User-Agent header required)
  api_blueprint->>ArWikiCats: Validate User-Agent header
  ArWikiCats-->>api_blueprint: Resolve English to Arabic label
  api_blueprint->>logs_db: "تصنيف:اليمن"
