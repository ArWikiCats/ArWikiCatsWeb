...
When code is pushed to the `main` branch, GitHub Actions triggers the deployment workflow. The workflow uses SSH to connect to the Toolforge server and executes [web_sh/update1.sh L1-L50](https://github.com/ArWikiCats/ArWikiCatsWeb/blob/88f42d13/web_sh/update1.sh#L1-L50)

 This script clones the repository to a temporary directory `$HOME/srcx`, copies the files to the target directory `$HOME/www/python/src`, activates the virtual environment, installs dependencies, and restarts the webservice.
