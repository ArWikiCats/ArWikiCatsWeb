...
1. **Always specify branch**: While `main` is the default, explicitly specify the branch to avoid confusion: ``` ```
2. **Check virtual environment**: Verify the virtual environment is activated when running dependency scripts: ``` ```
